...
class Graph(ABC):
    ...
    def construct_spatial_connection(self, temperature: float = 1.0, threshold: float = None,): # temperature must >= 1.0
        ...
        adjusted_logits = self.base_spatial_logits + self.spatial_logit_mlp(self.current_difficulty_tensor)
    ...
    def construct_spatial_connection_diff(self, round:int = 0, temperature: float = 1.0, threshold: float = None,): # temperature must >= 1.0
        ...
        adjusted_logits = self.base_spatial_logits[round] + self.spatial_logit_mlps[round](self.current_difficulty_tensor)
    ...
    def refine_masks_from_existing(self, pruning_rate: float, times: int = 1):
        if self.optimized_spatial:
            for i in range(self.rounds):
                for _ in range(times):  # apply multiple rounds
                    logits = self.base_spatial_logits[i] + self.spatial_logit_mlps[i](self.current_difficulty_tensor)
    ...
    def update_masks_diff(self, pruning_rate: float) -> torch.Tensor:
        if self.optimized_spatial:
            for i in range(self.rounds):
                ...
                prune_num_edges = torch.round(num_edges*pruning_rate) if torch.round(num_edges*pruning_rate)>0 else 1
                _edge_logits = self.base_spatial_logits[i] + self.spatial_logit_mlps[i](self.current_difficulty_tensor)
    ...
    def get_dynamic_logits(self):
        if not self.diff:
            spatial_logits = self.base_spatial_logits + self.spatial_logit_mlp(self.current_difficulty_tensor)
            temporal_logits = self.base_temporal_logits + self.temporal_logit_mlp(self.current_difficulty_tensor)
        else:
            spatial_logits = [
                self.base_spatial_logits[i] + self.spatial_logit_mlps[i](self.current_difficulty_tensor)
                for i in range(self.rounds)
            ]
            temporal_logits = [
                self.base_temporal_logits[i] + self.temporal_logit_mlps[i](self.current_difficulty_tensor)
                for i in range(self.rounds - 1)
            ]
    ...
    def compute_masks_from_scratch(self, pruning_rate: float):
        if self.optimized_spatial:
            for i in range(self.rounds):
                edge_logits = self.base_spatial_logits[i] + self.spatial_logit_mlps[i](self.current_difficulty_tensor)
