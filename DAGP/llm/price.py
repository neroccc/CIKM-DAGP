from DAGP.utils.globals import Cost, PromptTokens, CompletionTokens, Tokenizer, Deepseek_Tokenizer
import tiktoken
...
def cost_count_deepseek(prompt: str, response: str, model: str):
    # 删除分词器相关代码
    prompt_len = len(prompt) // 4  # 近似估算：1 token ≈ 4字符
    completion_len = len(response) // 4
    
    # Deepseek 定价标准（需确认最新价格）
    input_price_per_1k = 0.002  # 输入 $0.002/1k tokens
    output_price_per_1k = 0.008  # 输出 $0.008/1k tokens
    
    price = (prompt_len * input_price_per_1k / 1000) + \
            (completion_len * output_price_per_1k / 1000)
